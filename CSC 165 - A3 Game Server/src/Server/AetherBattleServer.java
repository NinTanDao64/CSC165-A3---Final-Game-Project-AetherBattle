...
public class AetherBattleServer extends GameConnectionServer<UUID> {
    private HashMap<UUID, Point3D> playerLocations;
    
    public AetherBattleServer(int localPort, ProtocolType protocolType) throws IOException {
    ...
    public void processPacket(Object o, InetAddress senderIP, int sndPort) {
        ...
        if (msgTokens.length > 0) {
            switch(msgTokens[0]) {
                ...
                case "bomb":
                	sendBombMsgs(UUID.fromString(msgTokens[1]));
                	break;
                ...
                case "spree":
                	sendSpreeMsgs(UUID.fromString(msgTokens[1]),
                			    Integer.parseInt(msgTokens[2]));
                	break;
                	
                case "hit":
                    sendHitMsg(UUID.fromString(msgTokens[1]), UUID.fromString(msgTokens[2]));
                    break;
            }
        }
    ...
    public void sendBombMsgs(UUID clientID) {
    	//Format bomb, clientID
    	try {
    		String msg = new String("bomb," + clientID.toString());
    ...
    public void sendSpreeMsgs(UUID clientID, int spree) {
    	try {
    		String msg = new String("spree," + clientID.toString());
    		msg += "," + spree;
    ...
    public void sendHitMsg(UUID shooterID, UUID woundedID) {
    	//Format hit, shooterID, woundedID
    	try {
    		String msg = new String("hit," + shooterID.toString());
    		msg += "," + woundedID;
